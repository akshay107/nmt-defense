        ...
        for num_steps, train_batch in enumerate(train_iter):
            ...
            if epoch>=int(args.epoch_start):
                ...
                for i in range(len(train_batch_new)):
                    # TODO: Check language and make target.
                    if 'run_en_fr' in args.input:
                        #print("Target language is french")
                        s = "Cette phrase n&apos; est pas correcte ."
                    if 'run_en_de' in args.input:
                        #print("Target language is german")
                        s = "Dieser Satz ist nicht korrekt ."
                    train_batch_new[i][1] = np.array([w2id[w] for w in s.split()],dtype='int32')
                    num_words = np.ceil(train_batch_new[i][0].shape[0]*frac_replace).astype('int')
                    if i==0:
                        print("num_words:",num_words)
                        print("target:",train_batch_new[i][1])
                    # TODO: bias or not
                    if args.bias == 'True':
                ...
                if args.fp16:
                    ...
                    optimizer.backward(float(args.lambda_val)*loss)
                else:
                    #(-0.1*loss).backward()
                    (float(args.lambda_val)*loss).backward()
                if epoch == -1 and args.grad_norm_for_yogi and args.optimizer == 'Yogi':
                    l2_norm += (utils.grad_norm(model.parameters()) ** 2) / n_params
                    continue
                num_grad_steps += 1

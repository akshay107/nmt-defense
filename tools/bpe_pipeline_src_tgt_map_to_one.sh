...
NAME="run_${src}_${tgt}_ep_${epoch_start}_emb_${emb_topk}_lambda_${lambda}_frac_${frac_replace}_con_${context}_top_${topk}_bias_${bias}_map_to_one"
OUT="temp/$NAME"
# OUT="temp/$NAME"
...
if [ ${#gpu} -eq 1 ]
then
CMD="python $TF/train_map_to_one.py -i $OUT/data --data processed \

...
#====== EXPERIMENT BEGIN ======
[ -d $OUT/output ] || mkdir -p $OUT/output
...
#      -t-valid valid.tgt \
#      -s-test test.src \
#      -t-test ${TF}/${OUT}/test/test.out.bpe \
#      --save_data processed \
#      --max_seq_len 70
#
# cp ${TF}/temp/backup_${NAME}/data/processed.vocab.pickle ${TF}/temp/${NAME}/data/processed.vocab.pickle

echo "Step 2: Train"
if [ ${#gpu} -eq 1 ]
then
CMD="python $TF/bruteforce_train.py -i $OUT/data --data processed \
--model_file $OUT/models/model_$NAME.ckpt --best_model_file $OUT/models/model_best_$NAME.ckpt \
--data processed --batchsize 1 --tied --beam_size 5 --epoch 40 \
...
--src $TF/$OUT/test.src.reduced.final \
--pred $TF/$OUT/test.out.bpe.reduced.final --size $size \
--out_file $OUT/output/$out_file --context $context --topk $topk --label_smoothing 0"
else
CMD="python $TF/bruteforce_train.py -i $OUT/data --data processed \
--model_file $OUT/models/model_$NAME.ckpt --best_model_file $OUT/models/model_best_$NAME.ckpt \
--data processed --batchsize 1 --tied --beam_size 5 --epoch 40 \
